...
import EditStationaryForm from "../form/form-elements/EditStationaryForm";


type StationaryDataTableProps = {
  stationary: any[];
  batch: any[];
  loading: boolean;
};

export default function StationaryDataTable({
  stationary,
  batch,
  loading,
}: StationaryDataTableProps) {
  const [selectedId, setSelectedId] = useState<string | null>(null);
  
  const [stationaryDetail, setStationaryDetail] = useState(false);
  const [stationaryData, setStationaryData] = useState<any>(null);


  console.log("get All Query To search in Course DATA Table:", stationary);


  const handleCloseModal = () => {
    setSelectedId(null);
    setStationaryDetail(false);
  ...
  const handleEditLab = (item: any) => {
    ...
    setStationaryDetail(true);
    setStationaryData(item);
  };
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        ...
        <div className="max-h-[500px] min-w-[1102px] overflow-y-auto">
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {stationary && stationary.length > 0 ? (
                stationary.map((item: any) => (
      ...
      {selectedId !== null && stationaryDetail === true && (
        <EditStationaryForm onCloseModal={handleCloseModal} stationaryData={stationaryData} />
      )}
    </div>
  );
}
