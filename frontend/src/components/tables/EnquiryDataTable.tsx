...
export default function EnquiryDataTable({
  ...
  const { followupDetails, isLoading, isError, refetch } = useFollowUp(selectedId);
  ...
  return (
    <div className="overflow-hidden rounded-md border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-950">
                  ...
                  return (
                    <TableRow key={item.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-900/40 transition-colors">
                      ...
                      <TableCell className="px-3 py-1.5">
                        ...
                        // className="px-1.5 py-0.5 text-[10px] font-semibold tracking-wide uppercase rounded-[3px]"
                        >
                          {item.leadStatus}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell className="px-3 py-1.5">
                        <div className="flex items-center gap-2">
                          <Tooltip isDisabled={!canWon} className="rounded bg-slate-800 text-[10px] text-white px-1.5 py-0.5" content="WON">
                            <button
                              disabled={!canWon}
                              className={`rounded p-0.5 transition ${!canWon ? "opacity-30 cursor-not-allowed" : "text-emerald-600 hover:bg-emerald-50 dark:text-emerald-400 dark:hover:bg-emerald-950/50"}`}
                              onClick={() => canMarkWon(item.leadStatus) && handleCompleteFollowUpHandler(item.id)}
                            >
                          ...
                          <Tooltip isDisabled={!canHold} className="rounded bg-slate-800 text-[10px] text-white px-1.5 py-0.5" content="HOLD">
                            <button
                              disabled={!canHold}
                              className={`rounded p-0.5 transition ${!canHold ? "opacity-30 cursor-not-allowed" : "text-amber-500 hover:bg-amber-50 dark:text-amber-400 dark:hover:bg-amber-950/50"}`}
                              onClick={() => canHoldEnquiry(item.leadStatus) && handleHoldEnquiryHandler(item.id)}
                            >
                          ...
                          <Tooltip isDisabled={!canLost} className="rounded bg-slate-800 text-[10px] text-white px-1.5 py-0.5" content="LOST">
                            <button
                              disabled={!canLost}
                              className={`rounded p-0.5 transition ${!canLost ? "opacity-30 cursor-not-allowed" : "text-rose-600 hover:bg-rose-50 dark:text-rose-400 dark:hover:bg-rose-950/50"}`}
                              onClick={() => canMarkLost(item.leadStatus) && handleLostEnquiryHandler(item.id)}
                            >
                      ...
                      <TableCell className="px-3 py-1.5">
                        <Tooltip className="rounded bg-slate-800 text-[10px] text-white px-1.5 py-0.5" content="Edit Enquiry">
                          <button
                            disabled={!canEdit}
                            className={`rounded p-0.5 transition ${!canEdit ? "opacity-30 cursor-not-allowed" : "text-slate-500 hover:bg-slate-100 dark:text-slate-400 dark:hover:bg-slate-800"}`}
                            onClick={() => canEditEnquiry(item.leadStatus) && handleEditEnquiry(item)}
                          >
      ...
      {/* === Modals unchanged for logical operation integrity === */}
      ...
      {modalType === "createNew" && selectedEnquiryId !== null && (
        <CreateNewFollowUpOnEnquiryModal enquiryId={selectedEnquiryId} title="Create Follow-Up" onClose={() => setModalType(null)} />
      )}
      {modalType === "editenquiry" && selectedEnquiryId !== null && (
        <EditEnquiryForm enquiryData={selectedEnquiryData} onCloseModal={() => setModalType(null)} courses={[]} />
      )}
      {modalType === "complete" && selectedEnquiryId !== null && (
        <CompleteFollowUpModal enquiryId={selectedEnquiryId} title="Complete Follow-Up" onClose={() => setModalType(null)} />
      )}
      {modalType === "hold" && selectedEnquiryId !== null && (
        <HoldEnquiryModal enquiryId={selectedEnquiryId} title="Hold Follow-Up" onClose={() => setModalType(null)} />
      )}
      {modalType === "lost" && selectedEnquiryId !== null && (
        <LostEnquiryModal enquiryId={selectedEnquiryId} title="Lost Follow-Up" onClose={() => setModalType(null)} />
      )}
    </div>
  );
}