...
import { getStationary } from "@/lib/api";
import { useSelector } from "react-redux";
import { RootState } from "@/store"; // Adjust path if needed
import { useDispatch } from "react-redux";
import React, { ChangeEvent, FormEvent, useState, useEffect, useCallback } from "react";
import { setStationaries, setCurrentPage, setSearchQuery, setTotal, setTotalPages } from "@/store/slices/stationarySlice";
import StudentCard from "@/components/common/StudentCard";
import { PAGE_SIZE } from "@/constants/pagination";
import useDebounce from "@/hooks/useDebounce";
import StationaryDataTable from "@/components/tables/StationaryDataTable";

export default function StationaryTable() {
  const [showForm, setShowForm] = useState(false);
  //const [enquiries, setEnquiries] = useState<any[]>([]);
  const batch = useSelector((state: RootState) => state.batch.batches ?? []);
  const stationary = useSelector((state: RootState) => state.stationary.stationaries)
  const [loading, setLoading] = useState<boolean>(false);
  const { currentPage, total, totalPages, searchQuery } = useSelector((state: RootState) => state.stationary);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        console.log("STATIONARY IN STATIONARY TABLE:", response);

        dispatch(setStationaries(response.stationary || []));
  ...
  return (
    <div>
      <div className="space-y-6">
        <StudentCard title="Stationary Lists">
          <Search
            value={searchInput}
