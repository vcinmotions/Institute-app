...
export const useEditFaculty = () => {
  ...
  console.log("get Token in useEditFaculty:", token);

  return useMutation({
    mutationFn: async ({ newFaculty, id }: { newFaculty: any; id: any }) => {
      if (!token) throw new Error("Missing token for create enquiry");

      dispatch(setLoading(true));
      await editFacultyAPI(token, newFaculty, id);
    ...
    onSuccess: async ({ token }) => {
      try {
        ...
        console.log(
          "get Faculty List after create new course:",
          updated,
          updated.faculty,
        );
